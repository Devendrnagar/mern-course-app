...
import { z } from 'zod';
import { CourseMatchInputSchema } from '@/lib/schemas';
...
export default function CourseMatchPage() {
  ...
  async function onSubmit(values: CourseMatchInput) {
    ...
    try {
      const response = await recommendCourses(values.description, 'Beginner');
      const suggestions = response.courses.map((course, index) => ({
        courseName: course.title,
        universityName: 'Recommended Track',
        matchScore: Math.max(70, 95 - index * 10),
        rationale: `Suggested for ${response.topic} learners at ${course.level} level.`,
      }));
